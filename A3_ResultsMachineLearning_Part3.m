...
[xtname, Ytname, yPred_Name, pVoxVoxSize, chunk_size, maskTypeNum, VideName, export_DIR] =  Aim3MachineLearningCaseSelection(CaseSelect)
...
configurations = {
    ...
    struct('iCreateVideo', "F", 'plot_Results', "TRUE", 'export_DIR', "TRUE"),     % case 3    % EXPORT + SHOW | NO VIDEO
    struct('iCreateVideo', "F", 'plot_Results', "F", 'export_DIR', "TRUE"),        % case 4    % EXPORT ONLY DATA   
    struct('iCreateVideo', "F", 'plot_Results', "TRUE", 'export_DIR', "F"),        % case 5    % ONLY SHOW VIDEO
...
for mltbI = 1:Show_True_Results
        ...
        if iCreateVideo == "TRUE"
            ...
            Video_FileName = join([ VideName,'.avi']);
...
for start = 1:   chunk_size:     DataSimulate   %     maxchunk      chunk_size*NumberRuns 
    ...
    %
    for j =    1:  1:  (size(y_train_Data, 2)-1)  % 52
    ...
    seconds  = mod( (idx(j)*15-15), 60)    ;
        ...
        exportMask = y_pred_Data(start:finish, j);
...
pause(.25)
           ...
           switch MaskType 
                ...
                case "Distance"
                        ...
                        %
                        %
                        mean_pred = mean(exportMask);
                        mean_stdev_pred = std(exportMask);
                        %exportMask = exportMask - ( abs(mean_train- mean_pred) +  abs(mean_stdev_train - mean_stdev_pred)*1.3 ); 
                        exportMask = exportMask ;
                        exportMask(exportMask > 0) = nan;
                        exportMask(exportMask < -20) = nan;
                ...
                case "ElectricField"
                        ...
                        mean_stdev_train = std(distancesIn);
                        %
                        %  maxOfLowest20Percent(distancesIn);
                        %  maxOfLowestXPercent(x, X)
                        ...
                        %exportMask = exportMask - ( abs(mean_train- mean_pred) +  abs(mean_stdev_train - mean_stdev_pred)*1.3 ); 
                        % Call the function
                        maxValueExportMask = maxOfLowestXPercent(exportMask, 93.5);
                        FilterExport = maxValueExportMask ;
                        %exportMask = exportMask - shiftExp ;
                        exportMask(exportMask > FilterExport) = nan;
                        exportMask(exportMask < 0) = nan;
    ...
    % 
        ...
        ExportSegmentation = "FALSE";
            ...
            %
            [fltrd_cords2, filtered_intensities2] = Global_remove_nan_intensity(QuerryPointsOG, exportMask);
            %
            C = compareIntensityMatrices(distancesIn, exportMask);
            C( C == 0) = nan;
...
%end 
  ...
  Show_True_Results = 1;
  NumberRuns = 1; 
  MaskType_All = ["Distance", "Binary", "Arrhenius"];
  MaskType = MaskType_All(   maskTypeNum   );
  shiftExp = 0; % 7.75;
  %
  %
...
for mltbI = 1:Show_True_Results
        ...
        if iCreateVideo == "TRUE"
            Video_Dir = "D:\VideoFiles\COMSOL Ablation Vectors\";
            Video_FileName = join([  "SimpleRegression-Model4",  "Ablation Mask_Comprehensive", ...
                                          VideName,'.avi']);
            Video_FileName = convertStringsToChars(Video_FileName);
            Video_fullfile = fullfile(Video_Dir, Video_FileName);
            videoWriter = VideoWriter(Video_fullfile); %// initialize the VideoWriter object
        ...
        All_Runs = floor(   (  size(X_train_Data, 1)  /chunk_size  )    )  *chunk_size ;
        %chunk_size = length(QuerryPointsOG);
        Volume_Difference_all = [];
        % Processing in increments of 2600 rows
        %   chunk_size*NumberRuns
        %   total_rows
...
%
%      total_rows         
      SelectRuns = chunk_size*NumberRuns; 
%
for start = 1:   chunk_size:    SelectRuns  %      
    ...
    for j =    4:4: 58
           ...
           distancesIn = y_train_Data(start:finish, j);
           exportMask = y_pred_Data(start:finish, j);
           %exportMask2 = matchHistograms(distancesIn, exportMask0);  
           %exportMask = exportMask2;



            % midpoint = [0,0, 35  ] ;
            % new_intensities = A3_Filter_NewIntensities_dSep( [QuerryPointsOG, distancesIn]  , midpoint, 100)  ;
            % distancesIn = new_intensities;
            % 
            % midpoint = [0,0, 35  ] ;
            % new_intensities_exp = A3_Filter_NewIntensities_dSep( [QuerryPointsOG, exportMask]  , midpoint, 100)  ;
            % exportMask = new_intensities_exp;
           ...
           switch MaskType 
               ...
               case "Arrhenius"
                        mean_train = mean(distancesIn );
                        mean_stdev_train = std(distancesIn);
                        distancesIn(distancesIn > 1) = nan;
                        distancesIn(distancesIn < .9) = nan;
                        %
                        %
                        %
                        mean_pred = mean(exportMask);
    ...
    figure(1)
        ...
        set(gcf,'color','w');                
           ...
           % filtered_intensities = distances;
            scatter3( P(:,1) , P(:,2) ,P(:,3), 10,  filtered_intensities, 'filled' ) 
                ...
                newHeight = currentPosition(4) / 1.5; % New height is half of the original
                newBottom = currentPosition(2) + (currentPosition(4) - newHeight)/2; % Adjust bottom to center
                cb.Position = [.45  newBottom currentPosition(3) newHeight];
                %         title(hc,'mm', 'FontSize', 20);
                title( join([ 'Ground Truth      Run:',  num2str(runNUM),   newline, ...
        ...
        subplot(1,2,2)
           P = fltrd_cords2;
           % filtered_intensities = distances;
            scatter3( P(:,1) , P(:,2) ,P(:,3), 10,  filtered_intensities2, 'filled' ) 
                ...
                %         title(hc,'mm', 'FontSize', 20);
                title( join([ 'ML Predicted      Run:',  num2str(runNUM), newline, ...
                              num2str(minutes), "Min", num2str(seconds), "s", newline,...
                                "Vol = ", Volume2, 'cm^{3}']), 'Fontsize', 18)
                C=caxis;
        axis equal
        %grid off
        %axis off
...
if export_DIR == "TRUE"
    exportDir = 'D:\Import To Matlab\01. Machine Learning Models Data\predict\';
    csvExportName = join([exportDir, ypred_fname, "Volume_pct_Difference.csv"]);
    csvwrite( csvExportName, Volume_Difference_all);
end 
