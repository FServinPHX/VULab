...
[k, area] = boundary(cubeX, cubeY, 1 );
%Cubuic Dimension = 1stDimention.^3/(repCM)
SingleDim = sqrt(area)*Scale;
...
Vol = (volume(shp1) + volume(shp2) - volume(shp3))/(CubicDimension*10);
%%
figure()
...
title( join(["3D Volume of Ablation Margin",...
    newline, "Volume =", num2str(round(Vol,1)) , "cm^3"    ])  )
