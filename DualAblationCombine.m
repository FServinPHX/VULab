...
function [ AllData ] = DualAblationCombine( TargetPoints1 , TargetPoints2, probe1,  probe2 ) 
...
for i=1:length(x2)
    ...
    NewAblation = [ x1, y1, z1; x2, y2, z2]; 
    AllDistances = [distances1, distances2]';

    % get the idx of points that intersect
    AllDistancesidx = AllDistances; 
    AllDistancesidx(AllDistancesidx < .5) = nan; 
    AllDistancesidx = AllDistancesidx./AllDistancesidx; 
    %
    AllData =  [NewAblation, AllDistances] .*AllDistancesidx ; 
