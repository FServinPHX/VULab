...
function [BoundaryPointsALL, all_liver_volumes,  all_liver_diameters  ]  = calculateArrheniusVolumeDiametersEllipseSymmetry(file_path)
%%

% file_path = "C:\Users\servinf\Documents\1.0 Vanderbilt\Dr. Miga Lab\Liver Project - Ablation - 2021\COMSOL_DATA\Heterogenous Data\Het - Cube Cloud- Paper 1\HalfDataCube\liver001_915MHZwTumorHeterogenousHalf21.2.csv"
% splitInHalf = "F";
% splitInHalfx = "T";
...
for patient_selection = 1:1
   ...
   baseline_data = readtable(file_path);
    %
    coordinates = baseline_data(:,1:2);
    temp_and_arrhenius = baseline_data(:,3:end);
    baseline_temp_array = [];
    baseline_arrhenius_array = [];
    
    BoundaryPoints.all = zeros(2000, 16*4*2);
    ...
    for ii = 1:1
        ...
        coordinates = all_data(:,1:2);
        temp_and_arrhenius = all_data(:,3:end);
        temp_array = [];
        arrhenius_array = [];
        for i  = 1:width(temp_and_arrhenius)
           if mod(i,2) == 1
...
%                ADDED 2023
                 ...
                 nec.temp = [];
                 for neci = 1:length(necrosis_points)
                     if necrosis_points(neci,1) ~= 0 && necrosis_points(neci,2) ~= 0
                           nec.temp = [nec.temp; necrosis_points(neci,:)];
                     end 
                  end
                     
                 %necrosis_points(necrosis_points(:,1) == 0 ) = [];
                 %necrosis_points = reshape(necrosis_points, [], 2);
                 necrosis_points =nec.temp; 
                ...
                if isempty(necrosis_points)
                    vol = 0;
                    xyz_diameters_temp = [0,0];
                    %inserting the boundary points 
                    BoundaryPoints.all(1,((i-1)*2+1):((i-1)*2+2)) = ...
                        [time(i) , arrhenius_thresh'];
                    BoundaryPoints.all(2,((i-1)*2+1):((i-1)*2+2)) = xyz_diameters_temp;
                else 
                    %finds the volume of the boundary
                    [~, vol] = boundary(necrosis_points,1);
                    ...
                    BoundaryPoints.all( 1:length( BoundaryPoints.kSortPoint )...
                        ,((i-1)*2+1):((i-1)*2+2)) = BoundaryPoints.kSortPoint;
                    %%%Mark the 
                    BoundaryPoints.all(1,((i-1)*2+1):((i-1)*2+2)) = ...
                        [time(i) , arrhenius_thresh'];
                    
                    xyz_diameters_temp = find_diameters2D(necrosis_points);                   
                end 
    ...
    export_All_liver_diams = [ string(name) , arrhenius_thresh'; baseline_diams];
...
% 
% 
% %     %Plotting Function scatter plot and TRISURF
% %     figure(1)
% %     plot3(BoundaryPoints.curr(:,1),BoundaryPoints.curr(:,2),...
% %        BoundaryPoints.curr(:,3), '.w', 'MarkerSize', 12)
% %     axis equal
% %     %view( -70, 40 );
% %     view( -95, 40 );
% %     P = [x, y, z];
% %     [k,volbound] = boundary(P,.1);
% %     hold on
% %     trisurf(k,P(:,1),P(:,2),P(:,3),'Facecolor',blue ,'FaceAlpha',0.9)
% %     hold off
% %     title( join(["Ablation Boundary (Boundary Function)",...
% %         newline, "Time = ",time(i) ,"(s)", ...
% %         "    Vol All = ", num2str( volbound/1000)]) )
% %     xlabel('X')
% %     ylabel('Y')
% %     zlabel('Z')
% %     set(gcf,'color','w');
% %     
% %     
% %     %Plotting Function scatter plot and ALPHA SHAPE    
% %     figure(2)
% %     plot3(BoundaryPoints.curr(:,1),BoundaryPoints.curr(:,2),...
% %        BoundaryPoints.curr(:,3), '.w', 'MarkerSize', 12)
% %     hold on
% %     plot(shpAll,'FaceColor', green ,'FaceAlpha',0.9) 
% %     hold off
% %     axis equal
% %     %view( -70, 40 );
% %     view( -95, 40 );
% %     title( join(["Ablation Boundary (Alpha Shape)", newline, "Time = ",time(i) ,"(s)",...
% %         "    Vol All = ", num2str( volume(shpAll)/1000)]) )
% %     xlabel('X')
% %     ylabel('Y')
% %     zlabel('Z')
% %     set(gcf,'color','w');
...
%             %Using the Boundary function to find the volume
%             P = [x, y, z];
%             [k,volRight] = boundary(P, 0);
%             
%             
...
% %             view( -80, 10 );
% %             title( join(["Ablation Boundary", newline, "Time = ",time(i) ,"(s)" ...
% %                 newline, "Vol All = ", num2str( volume(shpAll)/1000)  ]) )
% %             legend( "No Fat",join(["vol = ", num2str( volume(shpleft)/1000) ])...
% %                 , "Fat", join(["vol = ", num2str( volume(shpRight)/1000) ]))
% %             
% %             
% %             hold off
% %             set(gcf,'color','w');
...
%         radiiAll2 = [radiiAll2; [radii1(1)',radii3(2:3)' ] ]; 
% 
%         VolumeLeft = [ VolumeLeft, volume(shpleft)/1000  ];
%         VolumeRight = [ VolumeRight, volume(shpRight)/1000 ];
%         
%         VolumeLeft2 = [ VolumeLeft2, volleft/1000  ];
%         VolumeRight2 = [ VolumeRight2, volRight/1000 ];
% 
% 
%     end 
% 
...
% end
%  
% % %%Compare the trajctory of the alpha shape estimation vs the boundary
% % %%estimation
% % plot( [3:1:15*4+1] , VolumeLeft, '-g')
% % hold on 
% % plot( [3:1:15*4+1] , VolumeLeft2,'color', green)
% % plot( [3:1:15*4+1] , VolumeRight, '-r')
% % plot( [2:1:15*4+1] , VolumeRight2,'color', red)
% % title( join(["Volume Trajectory Comparison" ]))
% % legend( "Left_V_o_l Alpha", "Left_V_o_l Bound", "Right_V_o_l Alpha",...
% %     "Right_V_o_l Bound", 'Location', 'best')
% % set(gcf,'color','w');
% % set(gca,'FontSize',14)
% 
% BoundaryRadii = "T";
...
%         AblationVolumeLeft = array2table(AblationVolumeLeft);
%         
%         AblationVolumeRight = [string(name), arrhenius_thresh', VolumeRight ]';
%         AblationVolumeRight = array2table(AblationVolumeRight);
%         
%         %ABLATION VOLUME USING BOUNDARY FUNCTION
%         AblationVolumeLeft2 = [string(name), arrhenius_thresh', VolumeLeft2 ]';
%         AblationVolumeLeft2 = array2table(AblationVolumeLeft2);
%         
%         AblationVolumeRight2 = [string(name), arrhenius_thresh', VolumeRight2 ]';
%         AblationVolumeRight2 = array2table(AblationVolumeRight2);
%         
