...
I = imread("C:\Users\servinf\Documents\1.0 Vanderbilt\Dr. Miga Lab\Liver Ablation Phantom Study\Multiprobe 0 degree\All 4 Insertion Points highlighted_ 6 Checkeboard _ Cropped.jpeg")
I = rgb2gray(I);
% I = histeq(I);
...
I2 = imread("C:\Users\servinf\Documents\1.0 Vanderbilt\Dr. Miga Lab\Liver Ablation Phantom Study\Multiprobe 0 degree\TOP 2 segment Checker 6 Seg.png");
I2 = rgb2gray(I2);
% I2 = histeq(I2);
...
fileName = "C:\Users\servinf\Documents\1.0 Vanderbilt\Dr. Miga Lab\Liver Project - Ablation - 2021\COMSOL_DATA\Box Phantom Ablation\Multiprobe\Results\Boundary BoxGeomPhantomMultiprobe_TrueBeef20Degrees.csv";
...
for i = 15-3 : 4 :15*4
    ...
    for jj = 1:2%TODO Create a probe Model

        CenterProbe = [115+9.9, 170-30, 140; (115), 170-30, 140];
        ProbeRotate = [ angle, -angle ];
        addX = [-9.9, 9.9]*5*abs(angle/90); 
        ...
        disp(CenterProbe(jj, 1))

        newP = [R * [X,Y,Z]']' ;
        X = newP(:,1) + CenterProbe(jj, 1); 
        Y = newP(:,2) + CenterProbe(jj, 2); 
        Z = newP(:,3) + CenterProbe(jj, 3);

        R2 = rotz( 180 +ProbeRotate(jj) ); 
        newP2 = [R2 * [X,Y,Z]']' ;
        X = newP2(:,1) - ( mean(newP2(:,1)) -  CenterProbe(jj, 1) ) - addX(jj) ; %- addX(jj) ; 
        Y = newP2(:,2) - ( mean(newP2(:,2)) -  CenterProbe(jj, 2) )  ; 
        Z = newP2(:,3) - ( mean(newP2(:,3)) -  CenterProbe(jj, 3) ) ; 
...
%     set( p2, 'FaceColor', 'g' ,'FaceAlpha',.2, 'EdgeColor', 'none' );
    ...
    %Calculate the ablation time
    abTime.total = timex( (i-1)*timespacing +2) ;
    abTime.sec = mod(abTime.total,1)*(60/100)*100 ; 
    abTime.min = abTime.total - mod(abTime.total,1);
%     text(120, 160,160, join([num2str(abTime.min), 'm  ', num2str(abTime.sec), "s"]),...
%     'FontSize', 14)    
    
    
    title(join(["COMSOL MODEL", string( abs( angle) ), string( char(176)) ,...
        newline, " Fitted Ellipsoid ", newline, ...
        "Volume = ", round( (vol/1000) , 3) ,"cm^{3}", newline, ...
        "X_d = ", num2str(round(radii(2)*2,2)), "   |   ",...
        "Y_d = ", num2str(round(radii(1)*2,2)), "   |   ",...
        "Z_d = ", num2str(round(radii(2)*2,1)), newline, ...
        "Time = ", num2str(abTime.min), 'm  ', num2str(abTime.sec), "s"]) )
