...
close all
    ...
    %directoryPath =  'D:\ML COMSOL Models\COMSOL ZHighFat\Ablation Cloud 970'
    %directoryPath = 'D:\1.0 Vanderbilt\Dr. Miga Lab\3. Liver Project - Ablation - 2021\COMSOL\915 MHZ Collins Probe - 3D\Box Phantom\MultiProbe\Paper 3 All Runs\AblationCloud'
    directoryPath = 'D:\Import To Matlab\COMSOL Pointcloud ML_ALL Data\AllData\UnProcessed Ablation Grid'
    ...
    %exportDIR = 'D:\Import To Matlab\COMSOL Pointcloud ML_ALL Data   Binary Mask _ Distances  __ptIV';
    exportDIR = 'D:\Import To Matlab\COMSOL Pointcloud ML_ALL Data   Binary Mask _ Distances  __ptIV';
    %exportDIR = 'D:\ML COMSOL Models\COMSOL ZHighFat\Ablation Grid 970';
    %exportDIR = 'D:\1.0 Vanderbilt\Dr. Miga Lab\3. Liver Project - Ablation - 2021\COMSOL\915 MHZ Collins Probe - 3D\Box Phantom\MultiProbe\Paper 3 All Runs\AblationGrid'
...
%
start_files  = 1 ;
stop_files =  ceil( size(  FilesCSV, 1)/5 )     ; 

%
for fi = 1: size(  FilesCSV, 1)   %  size(  FilesCSV, 1)
    tic
% figure() 
...
DataExport = [];    
for i = 2:numChunks  % numChunks      %10: 33 
    ...
    % points = points(2:end, :);
    % plot3DScatter( points , 10, rgb("Purple")) 
    % axis equal
    % hold on
        ...
        if UpsamplingData == "TRUE"
                    ...
                    if Upsample == "TRUE"      
                        ...
                        if size(AllData1,1) < numpoints && sampleI< 10
                            [ AllData1 ] = UpsampledAblationSpec_CGBT( NewPoints, numpoints ) ;
                            disp("Upsampled") 
                            ...
                            if size(AllData1,1) < numpoints
                                while size(AllData1,1) < numpoints  && sampleI< 10
                                    [ AllData1 ] = UpsampledAblationSpec_CGBT( AllData1, numpoints ) ;
                                    disp("Upsampled II") 
                        ...
                        if  size(AllData1,1) > numpoints    &&  sampleI< 10
                            [ AllData1 ] = DownsampleAblationSpec_CGBT( NewPoints, numpoints ) ;
                             disp("Downsample") 
    ...
    %ProbePlacementInformation = "D:\1.0 Vanderbilt\Dr. Miga Lab\3. Liver Project - Ablation - 2021\COMSOL\915 MHZ Collins Probe - 3D\Box Phantom\MultiProbe\Paper 3 All Runs\ALL Experiment Dual Probe Placement Information_matlab.csv";
    %[ProbePointExport, plotedLine1, plotedLine2] =  A3_FindPlotPoints_CustomPlacements(file_path, ProbePlacementInformation)
    ...
    if plot_Filtered_Probe == "TRUE"
        ...
        scatter3( TargetPoints(:,1), TargetPoints(:,2), TargetPoints(:,3), 'k.');
...
%FINAL FILTER
    ...
    if plot_Results == "TRUE"
        ...
        % Your given character string
        str = fname;
        % Use regular expression to extract the experiment number
        % The pattern looks for the word 'Experiment' followed by one or more spaces and then captures the number
        exp_number = regexp(str, 'Experiment\s+(\d+)', 'tokens');
        % Convert from cell array of strings to a numeric value if found
        if ~isempty(exp_number)
            exp_number = str2double(exp_number{1}{1});
        else
            exp_number = [];
        end
        ...
        set(gcf,'color','w');                
           ...
           % filtered_intensities = distances;
            scatter3( P(:,1) , P(:,2) ,P(:,3), 10,  filtered_intensities, 'filled' ) 
                ...
                cb.Position = [currentPosition(1) newBottom currentPosition(3) newHeight];
                %         title(hc,'mm', 'FontSize', 20);
                title( join([ "Exp. ", exp_number, newline, "Vol = ", Volume, 'cm^{3}']), 'Fontsize', 30)
                C=caxis;
