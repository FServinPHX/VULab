...
function [MatrixPairSorted] = Aim3_LinkedPointCorrespond(A,B )
...
% Find the minimum numbers in each row
% Sort the minimum values in descending order
min_values = min( abs(dist_matrix) , [], 2);
% Select the highest 1000 minimum values
% Calculate the mean, median, and standard deviation of the highest 1000 minimum values
mean_high_min = mean(min_values);
median_high_min = median(min_values);
std_high_min = std(min_values);


min_dist = 0; 
max_dist = mean_high_min - std_high_min; 



while  min_dist < max_dist || pair_count<= num_points
...
for i = 1:num_pointsII
    ...
    if min_dist > mean_high_min + std_high_min*3

        dist_matrix = pdist2(pA, B_copy);
        min_dist = min(dist_matrix(:));
        [idx_A, idx_B] = find(dist_matrix == min_dist, 1);
        % Step 4: Record the actual point pair and delete the point values
        point_pair_AII = [point_pair_AII ;A(1,:)];
        point_pair_BII = [point_pair_BII ;B_copy(idx_B,:)];
        A(idx_A, :) = [];
