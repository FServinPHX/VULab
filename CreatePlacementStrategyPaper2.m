...
function [Arrange ] = CreatePlacementStrategyPaper2(angles, psiAngleArr, ...
    thetaAngleArr, spacing, TargetCentr, radiusSrt, VesselData, safetyMargin, ProbeColor,...
        Probelength )
...
for i = 1:spaceEnd
    ...
    x = [radius * cos(angles) ]';
    y = [radius * sin(angles) ]';
    z = [repmat(Probelength , 1, length(angles) )]';
...
%                     plot3(x(k) , y(k) , z(k) , '.', 'LineWidth', 1, 'MarkerSize', 10, 'Color',  'black' )
                    q1 = quiver3(x(k) , y(k) , z(k) , Vector(k,1) , Vector(k,2) , Vector(k,3) , ...
                         0, 'Color',  ProbeColor , 'LineWidth', 8 );
                         
                    if zprobecheck(pc) < 8
                        %colorNew(i, : )
%                         delete(q1)
                        plot3(x(k) , y(k) , z(k) , '.', 'LineWidth', 2, 'MarkerSize', 20, 'Color',  ProbeColor )
                        
                        quiver3(x(k) , y(k) , z(k) , Vector(k,1) , Vector(k,2) , Vector(k,3) , ...
                             0, 'Color',  ProbeColor, 'LineWidth', 7)
